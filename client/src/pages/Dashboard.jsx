    ...
    <div className="min-h-screen bg-white text-slate-950">
      <section className="relative isolate min-h-[640px] overflow-hidden bg-[#071d28] text-white sm:min-h-[700px]">
        ...
        <div className="mx-auto flex min-h-[640px] max-w-7xl items-center px-4 py-16 sm:min-h-[700px] sm:py-24">
          <Stagger trigger="mount" className="max-w-4xl">
            <Stagger.Item className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-3 py-1.5 text-[11px] font-extrabold backdrop-blur"><MapPin size={13} /> Manzanillo · México</Stagger.Item>
            <Stagger.Item as="h1" className="mt-7 max-w-4xl text-4xl font-black leading-[1.03] tracking-[-.055em] sm:text-6xl lg:text-7xl">Todo el ecosistema logístico de Manzanillo, en un solo lugar.</Stagger.Item>
            <Stagger.Item as="p" className="mt-6 max-w-3xl text-base leading-7 text-slate-200 sm:text-xl sm:leading-8">Encuentra transportistas, agentes aduanales, proveedores, fletes, vacantes, patios, talleres, noticias y herramientas para operar mejor.</Stagger.Item>

            <Stagger.Item as="form" onSubmit={search} className="mt-9 max-w-4xl rounded-2xl bg-white p-2 text-slate-900 shadow-2xl shadow-black/25 sm:flex">
            ...
            <Stagger.Item className="mt-4 flex max-w-4xl flex-wrap gap-2">{SEARCH_TYPES.map((item) => <button key={item} type="button" onClick={() => setType(item)} className={`press rounded-full border px-3 py-1.5 text-[10px] font-extrabold backdrop-blur transition ${type === item ? 'border-white bg-white text-[#081f2c]' : 'border-white/25 bg-black/10 text-slate-200 hover:border-white/50'}`}>{item}</button>)}</Stagger.Item>

            <Stagger.Item className="mt-8 flex flex-col gap-3 sm:flex-row">
              <Link to="/directorio-empresarial" className="press inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-white px-5 text-sm font-extrabold text-[#081f2c]">Explorar empresas <ArrowRight size={16} /></Link>
              <Link to="/fletes" className="press inline-flex min-h-12 items-center justify-center rounded-xl border border-white/30 bg-white/10 px-5 text-sm font-extrabold backdrop-blur hover:bg-white/15">Publicar un flete</Link>
              <Link to="/register" className="press inline-flex min-h-12 items-center justify-center rounded-xl border border-white/30 px-5 text-sm font-extrabold hover:bg-white/10">Registrar mi empresa</Link>
            </Stagger.Item>
          </Stagger>
        </div>
      </section>
      ...
      <main>
        <section className="border-b border-slate-100 py-16 sm:py-20">
          <div className="mx-auto max-w-7xl px-4">
            <Reveal><Heading eyebrow="Accesos rápidos" title="Encuentra lo que necesitas" text="Explora empresas y servicios por actividad dentro de la cadena logística." /></Reveal>
            <Stagger className="mt-9 grid grid-cols-2 border-l border-t border-slate-200 sm:grid-cols-4">
              {CATEGORIES.map(({ name, detail, icon: Icon, to }) => <MotionLink key={name} variants={staggerItem} to={to} className="group min-h-40 border-b border-r border-slate-200 p-5 transition hover:z-10 hover:border-teal-700 hover:shadow-[0_12px_34px_rgba(8,31,44,.08)] sm:p-6"><Icon size={22} strokeWidth={1.7} className="text-teal-800" /><h2 className="mt-6 text-sm font-black sm:text-base">{name}</h2><p className="mt-1 text-xs leading-5 text-slate-500">{detail}</p><ChevronRight size={14} className="mt-4 text-slate-300 transition group-hover:translate-x-1 group-hover:text-teal-700" /></MotionLink>)}
            </Stagger>
          </div>
        </section>
        ...
        <section className="border-b border-slate-100 py-16 sm:py-20">
          <Reveal className="mx-auto grid max-w-7xl gap-12 px-4 lg:grid-cols-[.8fr_1.2fr] lg:items-start">
            ...
            <Stagger className="grid gap-x-8 gap-y-7 sm:grid-cols-2">{[
            ...
            ].map(([Icon, title, text]) => <Stagger.Item key={title} className="border-t border-slate-200 pt-5"><Icon size={20} className="text-teal-800" /><h3 className="mt-4 font-black">{title}</h3><p className="mt-2 text-sm leading-6 text-slate-500">{text}</p></Stagger.Item>)}</Stagger>
          </Reveal>
        </section>

        <section className="border-b border-slate-100 bg-[#f5f7f8] py-16 sm:py-20">
          <div className="mx-auto max-w-7xl px-4">
            <Reveal className="grid gap-12 lg:grid-cols-[.9fr_1.1fr] lg:items-start">
              <div><Heading eyebrow="Operación conectada" title="Rutas inteligentes y aduanas bajo control." text="Faro Portuario combina reglas vehiculares, riesgos, fuentes GPS autorizadas y seguimiento aduanal para anticipar bloqueos y retrasos con información trazable." /><Link to="/torre-control" className="press mt-7 inline-flex min-h-11 items-center gap-2 rounded-xl bg-[#0d4f4b] px-5 text-xs font-extrabold text-white">Conocer la Torre de Control <ArrowRight size={14}/></Link><p className="mt-4 text-xs leading-5 text-slate-500">La disponibilidad y frecuencia dependen de las fuentes oficiales, proveedores GPS, agentes aduanales, terminales y servicios conectados.</p></div>
              ...
              ].map(([Icon,title,copy])=><Stagger.Item key={title} className="rounded-xl border border-slate-200 bg-white p-5"><Icon size={19} className="text-teal-700"/><h3 className="mt-4 text-sm font-black">{title}</h3><p className="mt-2 text-xs leading-5 text-slate-500">{copy}</p></Stagger.Item>)}</Stagger>
            </Reveal>
          </div>
        </section>
        ...
        <section className="border-b border-slate-100 py-16 sm:py-20">
          <div className="mx-auto max-w-7xl px-4">
            <Reveal className="grid gap-10 lg:grid-cols-2">
              <div><Heading eyebrow="Actualidad" title="Información útil, con fuente y fecha" text="Noticias, normatividad, capacitación y alertas editoriales publicadas con contexto verificable." /><Link to="/noticias" className="press mt-7 inline-flex min-h-11 items-center gap-2 rounded-xl border border-slate-200 px-4 text-xs font-extrabold">Consultar actualidad <Newspaper size={14} /></Link></div>
              <div><Heading eyebrow="Planes comerciales" title="Vende, contrata o anuncia tu empresa" text="Consulta precios claros para Marketplace, publicación de vacantes y campañas publicitarias. El registro de cuenta es gratuito." /><div className="mt-7 flex flex-wrap gap-3"><Link to="/marketplace#planes" className="press inline-flex min-h-11 items-center gap-2 rounded-xl bg-[#0d4f4b] px-5 text-xs font-extrabold text-white">Ver membresías <ArrowRight size={14} /></Link><Link to="/anunciate" className="press inline-flex min-h-11 items-center gap-2 rounded-xl border border-slate-200 px-5 text-xs font-extrabold">Paquetes de publicidad</Link></div></div>
            </Reveal>
          </div>
        </section>

        <section className="border-b border-slate-100 py-16 sm:py-20">
          <Reveal className="mx-auto max-w-4xl px-4"><Heading eyebrow="Preguntas frecuentes" title="Antes de comenzar" text="Respuestas claras sobre participación, publicación y validación." /><div className="mt-8 divide-y divide-slate-200 border-y border-slate-200">{[
          ...
          ].map(([question, answer]) => <details key={question} className="group py-5"><summary className="flex cursor-pointer list-none items-center justify-between gap-4 text-sm font-black"><span>{question}</span><span className="text-xl font-light text-slate-400 group-open:rotate-45">+</span></summary><p className="mt-3 max-w-3xl text-sm leading-6 text-slate-500">{answer}</p></details>)}</div></Reveal>
        </section>

        <section className="py-16 sm:py-24"><div className="mx-auto max-w-7xl px-4"><Reveal className="rounded-3xl bg-[#081f2c] px-6 py-11 text-white sm:px-12 sm:py-14 lg:flex lg:items-center lg:justify-between"><div className="max-w-3xl"><p className="text-[11px] font-black uppercase tracking-[.2em] text-teal-300">Faro Portuario</p><h2 className="mt-4 text-3xl font-black tracking-[-.04em] sm:text-4xl">Forma parte del ecosistema logístico de Manzanillo.</h2><p className="mt-4 text-sm leading-6 text-slate-300">Crea tu cuenta y construye una presencia profesional con información real.</p></div><div className="mt-8 flex flex-col gap-3 sm:flex-row lg:mt-0"><Link to="/register" className="press rounded-xl bg-white px-6 py-3 text-center text-sm font-extrabold text-[#081f2c]">Crear cuenta</Link><Link to="/directorio-empresarial" className="press rounded-xl border border-white/25 px-6 py-3 text-center text-sm font-extrabold">Explorar directorio</Link></div></Reveal></div></section>
      </main>
    </div>
  )
}
