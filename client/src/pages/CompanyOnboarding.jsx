      ...
      <div className="mx-auto max-w-5xl">
        ...
        <div className="mt-8 grid gap-10 lg:grid-cols-[220px_1fr]">
          ...
          <section className="rounded-3xl border border-slate-200 p-5 shadow-[0_16px_50px_rgba(8,31,44,.06)] sm:p-8">
            ...
            {step === 4 && <Step title="Documentación legal privada" text="PDF, JPG, PNG o WebP. Máximo 10 MB por archivo. Solo el equipo autorizado de validación puede consultarlos."><div className="space-y-3">{requiredDocuments.map(([type, label]) => <DocumentUpload key={type} label={label} uploaded={documents.find((item) => item.type === type)} disabled={loading} onFile={(file) => uploadDocument(type, file)} />)}</div><p className="mt-4 text-[11px] leading-5 text-slate-500">Todos los documentos deben aprobarse antes de publicar o solicitar un flete. Faro verifica identidad y documentación; no sustituye a una autoridad.</p></Step>}
