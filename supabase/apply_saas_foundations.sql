-- ════════════════════════════════════════════════════════════════════════════
-- FARO PORTUARIO · apply_saas_foundations.sql (idempotente, seguro para prod)
--
-- Arregla el error del onboarding empresarial:
--   "Could not find the function public.create_company_draft(...)"
--
-- Contiene:
--   0) Prerrequisito: garantiza que exista public.subscriptions (migración 006)
--   1) Migración 015 completa (companies, roles, permisos, RPCs, RLS, bucket)
--
-- Todo usa `if not exists` / `on conflict` / `create or replace` /
-- `drop policy if exists`, por lo que se puede ejecutar varias veces sin dañar
-- datos existentes. Pégalo completo en el SQL Editor de Supabase y ejecútalo.
-- ════════════════════════════════════════════════════════════════════════════

-- ── 0. Prerrequisito: tabla subscriptions (de migración 006) ────────────────
-- El paso `alter table subscriptions add column ...` de la 015 requiere que la
-- tabla ya exista. Este bloque la crea solo si falta (no toca datos si ya está).
create table if not exists public.subscriptions (
  id            uuid primary key default gen_random_uuid(),
  user_id       uuid references auth.users(id) on delete cascade,
  tipo          text not null default 'empresa_vacantes',
...
drop policy if exists "User inserts own subscription" on public.subscriptions;
create policy "User inserts own subscription"
  on public.subscriptions for insert
  with check (auth.uid() = user_id and estatus = 'pendiente');

drop policy if exists "User updates own pending subscription" on public.subscriptions;
...
-- ── 1. Migración 015 (fundamentos SaaS) ─────────────────────────────────────
...
drop policy if exists "account_types_public_read" on public.account_types;
...
drop policy if exists "plan_limits_authenticated_read" on public.plan_limits;
...
drop policy if exists "companies_public_verified" on public.companies;
...
drop policy if exists "company_members_own_read" on public.company_members;
...
drop policy if exists "company_documents_member" on public.company_documents;
...
drop policy if exists "company_documents_member_insert" on public.company_documents;
...
drop policy if exists "company_verifications_member_read" on public.company_verifications;
...
drop policy if exists "audit_authorized_read" on public.audit_logs;
